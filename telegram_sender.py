...
def build_message(matched: list[tuple[dict[str, Any], list[str]]]) -> str:
    ...
    summary_parts = []
    for kw in KEYWORDS:
        count = sum(1 for item, kws in matched if kw in kws)
        summary_parts.append(f"{kw} {count}건")

    lines.append(f"<b>요약: {', '.join(summary_parts)}</b>\n")
...
def send_message(token: str, chat_id: str, text: str) -> None:
    """텔레그램 API를 호출하여 메시지를 전송한다 (지수 백오프 기반 재시도)."""
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            log.warning(f"텔레그램 전송 중 예외 발생: {e}")

        if attempt < MAX_RETRIES:
            wait = BACKOFF_BASE_SECONDS * (2**backoff_exponent)
            log.info(f"{wait}초 후 재시도... ({attempt}/{MAX_RETRIES})")
            time.sleep(wait)
            backoff_exponent += 1
...
def send_alert(matched: list[tuple[dict[str, Any], list[str]]]) -> None:
    """모든 프로세스를 묶어 텔레그램 알람을 전송한다."""
    try:
        token, chat_id = load_config()
    except RuntimeError as e:
        logger.get_logger().error(f"텔레그램 설정 오류: {e}")
        return
